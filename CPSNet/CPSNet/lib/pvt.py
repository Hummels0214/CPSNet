...
class PolypPVT(nn.Module):
    def __init__(self, channel=32,
        ...
        self.dmf = DMF(channel)
        self.ca = ChannelAttention(64)
        self.sa = SpatialAttention()
        self.mfem = MFEM()

        self.down05 = nn.Upsample(scale_factor=0.5, mode='bilinear', align_corners=True)
        self.out_MFEM = nn.Conv2d(channel, 1, 1)
        self.out_DMF = nn.Conv2d(channel, 1, 1)
        self.cdm = CDM(64, 64)
        self.DCA = DCA(n=n,
    ...
    def forward(self, x):
        ...
        cfm_feature1, cfm_feature2 = self.dmf(x4_t, x3_t, x2_t, x2_d, x3_d)

        # SAM
        T2 = self.Translayer2_0(ctm_feature)
        T2 = self.down05(T2)
        sam_feature1 = self.mfem(cfm_feature1, T2)  # main
        sam_feature2 = self.mfem(cfm_feature2, T2)
        # sam_feature3 = self.tem(cfm_feature3, T2)

        prediction1 = self.out_MFEM(sam_feature1)  # main
        prediction2 = self.out_MFEM(sam_feature2)
        # prediction3 = self.out_SAM(sam_feature3)
        prediction3 = self.out_DMF(cfm_feature1)  # main
        prediction4 = self.out_DMF(cfm_feature2)
