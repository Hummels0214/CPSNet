...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    torch.save(model.state_dict(), save_path + str(epoch) + 'Camouflaged.pth')
    # choose the best model

    global dict_plot

    test1path = './dataset/COD-TestDataset/'
    if (epoch + 1) % 1 == 0:
        ...
        if meandice > best:
            best = meandice
            torch.save(model.state_dict(), save_path + 'Camouflaged.pth')
            torch.save(model.state_dict(), save_path + str(epoch) + 'Camouflaged.pth')
            print('##############################################################################best', best)
            logging.info(
                '##############################################################################best:{}'.format(best))
...
if __name__ == '__main__':
    dict_plot = {'CAMO': [], 'CHAMELEON': [], 'COD10K': [], 'NC4K': [], 'test': []}
    name = ['CAMO', 'CHAMELEON', 'COD10K', 'NC4K', 'test']
    ##################model_name#############################
    model_name = 'Camouflaged-1.25'
    ###############################################11
    ...
    parser.add_argument('--train_path', type=str,
                        default='./dataset/COD-TrainDataset/',
                        help='path to train dataset')

    parser.add_argument('--test_path', type=str,
                        default='./dataset/COD-TestDataset/',
    ...
    model = PolypPVT().cuda()
    # model = PolypPVT()
    # model.load_state_dict(torch.load('/home/ti-2/Desktop/6.28restart/7.17/Polyp-PVT-main/27PolypPVT-best.pth'))
    # model.cuda()
    best = 0
    ...
    schedular = CosineAnnealingLR(optimizer, T_max=25, eta_min=2.5e-5)
    print(optimizer)
    ...
    for epoch in range(1, opt.epoch):
        # adjust_lr(optimizer, opt.lr, epoch, 0.5, 50)
